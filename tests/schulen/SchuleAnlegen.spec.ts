...
test.describe(`Testfälle für das Anlegen von Schulen als Landesadmin: Umgebung: ${process.env.ENV}: URL: ${process.env.FRONTEND_URL}:`, () => {
  ...
  test(`Schulen Anlage: UI prüfen`, { tag: [LONG, SHORT, STAGE, BROWSER] },  async () => {
    await schuleCreationPage.checkCreateForm();
  });

  // SPSH-2954
  test(`Eine Schule anlegen als Landesadmin und die Bestätigungsseite vollständig prüfen`, { tag: [LONG, SHORT, DEV, STAGE, BROWSER] },  async () => {
  ...
  // SPSH-2952
  test(`2 Schulen nacheinander anlegen als Landesadmin`, { tag: [LONG, SHORT, STAGE, BROWSER] },  async () => {
