...
test.describe(`Testfälle für die Administration von Personen": Umgebung: ${process.env.UMGEBUNG}: URL: ${process.env.FRONTEND_URL}:`, () => {
  ...
  test("Einen Benutzer mit der Rolle Lehrkraft anlegen", async ({ page }) => {
    ...
    const Nachname =
      "TAutoN" + faker.person.lastName() + "-" + faker.person.lastName(); // Wahrscheinlichkeit doppelter Namen verringern
    ...
    await test.step(`Benutzer anlegen`, async () => {
      ...
      Benutzername =
        await PersonCreationView.text_Neuer_Benutzername.innerText();
      Einstiegspasswort =
  ...
  test("Prüfung auf ungültige Organisationen bei Anlage Benutzer", async ({
    ...
    await test.step(`Rolle Landesadmin auswaehlen und verfügbare Einträge für die Organisationsebene prüfen(Gültige Organisationen: ROOT, LAND)`, async () => {
      ...
      const response_schulstrukturknoten = await page.waitForResponse(
        (response) =>
          response.url().includes("/api/personenkontext/schulstrukturknoten")
      );
  ...
  test("In der Ergebnisliste die Suchfunktion ausführen", async ({ page }) => {
    ...
    await test.step(`Benutzerverwaltung öffnen und Suche nach Vornamen `, async () => {
      ...
      await PersonManagementView.input_Suchfeld.fill("Yvonne");
      await PersonManagementView.button_Suchen.click();
      await expect(
        page.getByRole("cell", { name: "yvonbreitenfelshohenroehr" })
      ).toBeVisible();
    });

    await test.step(`Suche nach Nachnamen `, async () => {
      await PersonManagementView.input_Suchfeld.fill(
        "von Breitenfels-Hohenröhr"
      );
      await PersonManagementView.button_Suchen.click();
      await expect(
        page.getByRole("cell", { name: "yvonbreitenfelshohenroehr" })
    ...
    await test.step(`Suche nach Benutzernamen `, async () => {
      await PersonManagementView.input_Suchfeld.fill(
        "yvonbreitenfelshohenroehr"
      );
      await PersonManagementView.button_Suchen.click();
      await expect(
        page.getByRole("cell", { name: "von Breitenfels-Hohenröhr" })
      ).toBeVisible();
    });
