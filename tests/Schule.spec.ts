...
test.describe(`Testfälle für die Administration von Schulen: Umgebung: ${process.env["UMGEBUNG"]}: URL: ${process.env["FRONTEND_URL"]}:`, () => {
  ...
  test("Ergebnisliste Schulen auf Vollständigkeit prüfen als Landesadmin @long @short @stage", async ({
    ...
    await test.step(`Schulverwaltung öffnen und Alle Elemente in der Ergebnisliste auf Existenz prüfen`, async () => {
      ...
      await expect(
        SchuleManagementView.table_header_Dienststellennummer,
      ).toBeVisible();
  ...
  test("Eine Schule anlegen als Schuladmin und die Bestätigungsseite vollständig prüfen @long @short", async ({
    ...
    await test.step(`Testdaten: Schuladmin anlegen und mit diesem anmelden`, async () => {
      ...
      userInfo = await createPersonWithUserContext(
        ...
        "LEIT",
        "TAuto-PW-B-MeierLEIT",
        "TAuto-PW-B-Hans",
        idSP,
        "TAuto-PW-R-RolleLEIT",
      );
    ...
    await test.step(`Bestätigungsseite prüfen`, async () => {
      ...
      await expect(SchuleCreationView.text_DatenGespeichert).toHaveText(
        "Folgende Daten wurden gespeichert:",
      );
      await expect(SchuleCreationView.label_Schulform).toHaveText("Schulform:");
      await expect(SchuleCreationView.data_Schulform).toHaveText(
        "Öffentliche Schule",
      );
      await expect(SchuleCreationView.label_Dienststellennummer).toHaveText(
        "Dienststellennummer:",
      );
      await expect(SchuleCreationView.data_Dienststellennummer).toHaveText(
        DIENSTSTELLENNUMMER,
      );
      ...
      await expect(
        SchuleCreationView.button_WeitereSchuleAnlegen,
      ).toBeVisible();
      await expect(
        SchuleCreationView.button_ZurueckErgebnisliste,
      ).toBeVisible();
    });
