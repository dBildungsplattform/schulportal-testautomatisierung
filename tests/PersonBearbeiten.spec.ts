  ...
  test("Eine Schulzuordnung bei einem bestehenden Benutzer hinzufügen", {tag: [LONG, STAGE]}, async ({ page }) => {
    ...
    test("Befristung beim hinzufügen von Personenkontexten", { tag: [LONG, STAGE] }, async ({ page }) => {
        ...
        await test.step(`Befristung bei ${unbefristeteRolle} und ${befristeteRolle} überprüfen`, async () => {
            ...
            await expect(personDetailsView.radioButton_befristungSchuljahresende).toBeChecked({ timeout: 1000 });
        
            await personDetailsView.rollen.selectByTitle(unbefristeteRolle);
            await expect(personDetailsView.radioButton_befristungUnbefristet).toBeChecked({ timeout: 1000 });
        });
    })
