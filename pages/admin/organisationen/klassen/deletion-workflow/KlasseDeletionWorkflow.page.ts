...
export class KlasseDeletionWorkflowPage {
  ...
  constructor(
    ...
    protected readonly originPage: KlasseManagementViewPage | KlasseDetailsViewPage,
  ) {}
  ...
  public async deleteKlasse(schulname: string, klassenname: string): Promise<void> {
    ...
    await expect(this.page.getByTestId('klasse-delete-confirmation-text')).toHaveText(
      `Wollen Sie die Klasse ${klassenname} an der Schule ${schulname} wirklich entfernen?`,
    );
  ...
  public async klasseSuccessfullyDeleted(schulname: string, klassenname: string): Promise<KlasseManagementViewPage> {
    await expect(this.page.getByTestId('klasse-delete-success-text')).toHaveText(
      `Die Klasse ${klassenname} an der Schule ${schulname} wurde erfolgreich gelöscht.`,
    );
  ...
  public async klasseDeletionFailed(): Promise<KlasseManagementViewPage | KlasseDetailsViewPage> {
    await expect(this.page.getByTestId('klasse-details-error-alert-text')).toHaveText(
